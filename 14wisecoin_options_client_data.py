...
async def get_all_option_symbols():
    ...
    try:
        ...
        # 加载实时监控标的
        live_symbols = []
        if os.path.exists("wisecoin-symbol-live.json"):
            try:
                with open("wisecoin-symbol-live.json", "r", encoding="utf-8") as f:
                    live_symbols = json.load(f)
                logger.info(f"加载监控标的列表: {len(live_symbols)} 个")
            except Exception as e:
                logger.warning(f"加载 wisecoin-symbol-live.json 失败: {e}")
        ...
        # 仅获取在 wisecoin-symbol-live.json 中配合的合约行情
        if live_symbols:
            if 'underlying_symbol' in symbol_info_df.columns:
                symbol_info_df = symbol_info_df[symbol_info_df['underlying_symbol'].isin(live_symbols)]
                filtered_options = symbol_info_df.index.tolist()
                logger.info(f"在 {len(live_symbols)} 个监控标的中匹配到 {len(filtered_options)} 个期权合约")
            else:
                logger.warning("合约信息中未找到 underlying_symbol 列，无法进行过滤")
...
async def get_underlying_futures_quotes():
    ...
    """
    QUOTE_EXCEL_FILE = os.path.join(TEMP_DIR, "wisecoin-期权行情.xlsx")
    FUTURES_EXCEL_FILE = os.path.join(TEMP_DIR, "wisecoin-期货行情.xlsx")
    ...
    try:
        ...
        if not unique_underlyings:
            ...
            # 注意：即使没有期权标的，后续因为要加金融期货，所以不直接 return，而是初始化为空列表继续
            unique_underlyings = []

        # -----------------------------------------------------------
        # [新增] 集成金融期货 (股指期货 + 国债期货)
        # 参考 01wisecoin-options-ranking.py 的逻辑
        # -----------------------------------------------------------
        try:
            logger.info("正在获取 CFFEX 金融期货(股指/国债)...")
            # 获取所有期货
            all_futures_list = await api.query_quotes(ins_class='FUTURE', expired=False)
            ...
            financial_futures = []
            for s in all_futures_list:
                for prefix in financial_prefixes:
                    if s.startswith(prefix):
                        financial_futures.append(s)
                        break
            
            logger.info(f"获取到 {len(financial_futures)} 个金融期货合约。")
            
            # 合并到 unique_underlyings (去重)
            unique_underlyings.extend(financial_futures)
            unique_underlyings = sorted(list(set(unique_underlyings)))
            
        except Exception as e:
            logger.warning(f"获取金融期货失败: {e}")
        # -----------------------------------------------------------
            
        if not unique_underlyings:
            logger.warning("未获取到任何有效的期货合约（期权标的 或 金融期货）。")
            return

        logger.info(f"最终获取到 {len(unique_underlyings)} 个标的期货合约(含金融期货)，准备同步详细信息和行情...")

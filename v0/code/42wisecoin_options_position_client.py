...
class OptionTShapeWindow(QMainWindow):
    ...
    def load_data(self):
        ...
        try:
            ...
            # 读取持仓数据
            positions_file = os.path.join(self.temp_dir, 'wisecoin-持仓.xlsx')
            if not os.path.exists(positions_file):
                positions_file = 'wisecoin-持仓.xlsx'
            if os.path.exists(positions_file):
                self.positions_df = pd.read_excel(positions_file)
            else:
                self.positions_df = pd.DataFrame()
                print("未找到持仓文件: wisecoin-持仓.xlsx")
